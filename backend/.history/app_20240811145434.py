...
# adding a test document to the databse lol
@app.route('/add-test', methods=['GET'])
def add_test_document():
    test_data = {
        "name": "Test Document",
        "description": "This is a test document added to the database."
    }
    result = db.mycollection.insert_one(test_data)
    return jsonify({"_id": str(result.inserted_id), "message": "Test document added"})
